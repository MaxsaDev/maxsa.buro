...
**Проблема:** Сайдбар завжди показував статичний блок з назвою застосунку "MAXSA SP". Після введення системи офісів (філій) стало необхідним показувати активний офіс користувача та надати можливість швидкого перемикання між офісами прямо з сайдбара, не заходячи в налаштування.

**Рішення:** Умовне відображення у шапці сайдбара (`<SidebarHeader>`):

- На сторінках `/mx-admin/*` або коли користувачу не призначено жодного офісу → статичний блок "MAXSA SP"
...
!isAdminRoute && userOffices.length > 0
  ...
  → статичний блок "MAXSA SP"  (адмін або немає офісів)
```
...
<SidebarHeader>
  {!isAdminRoute && userOffices.length > 0 ? (
    ...
    // Статичний блок "MAXSA SP" — без змін
  )}
</SidebarHeader>
```
